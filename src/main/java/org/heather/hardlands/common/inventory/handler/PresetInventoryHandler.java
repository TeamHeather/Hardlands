...
import org.heather.hardlands.module.PresetRepository.PresetInfo;
...
public final class PresetInventoryHandler implements InventoryHandler {
    ...
    public void render(Inventory inventory) {
        List<PresetInfo> presets = getRepository().getPresets();
        ...
        inventory.setItem(HardlandsInventory.contentSlot(SAVE_INDEX), createSaveItem());

        int start = this.page * pageSize;

        for (int index = 0; index < pageSize && start + index < presets.size(); index++) {
            inventory.setItem(HardlandsInventory.contentSlot(index + 1), createPresetItem(presets.get(start + index)));
        }
        ...
        if (this.page < lastPage) {
            inventory.setItem(HardlandsInventory.contentSlot(capacity - NEXT_OFFSET), InventoryItem.NEXT.build());
        }
    }
    ...
    public Optional<Boolean> onClick(InventoryClickEvent event, Player player) {
        ...
        if (index == SAVE_INDEX) {
            this.showSaveDialog(player, inventory, "", PresetRepository.DEFAULT_ICON.getKey().asString(), "", null);
        ...
        if (event.isLeftClick()) {
            ...
            player.sendRichMessage("<green>Preset <white>%s</white> aplicado.".formatted(preset.name()));
            return Optional.of(true);
        }
    ...
    private void showSaveDialog(Player player, Inventory inventory, String name, String icon, String description, String error) {
        DialogBase.Builder base = DialogBase.builder(Component.text("Crear preset"))
                ...
                .inputs(List.of(
                        DialogInput.text(NAME_INPUT, Component.text("Nombre"))
                                ...
                                .maxLength(32)
                                .build(),
                        DialogInput.text(ICON_INPUT, Component.text("Icono"))
                                .width(300)
                                .initial(icon)
                                .maxLength(128)
                                .build(),
                        DialogInput.text(DESCRIPTION_INPUT, Component.text("Descripción"))
                                .width(400)
                                .initial(description)
                                .maxLength(256)
        ...
        if (error != null) base.body(List.of(DialogBody.plainMessage(Component.text(error))));
        ...
        player.showDialog(Dialog.create(builder -> builder
                .empty()
                .base(base.build())
                .type(DialogType.confirmation(save, createCancelButton()))));
    }
    ...
    private void showEditDialog(
        ...
        ActionButton delete = ActionButton.create(
                ...
                DialogAction.customClick((response, audience) -> {
                    ...
                    this.render(inventory);
                    target.closeDialog();
                    target.sendRichMessage("<red>Preset <white>%s</white> eliminado.".formatted(preset.name()));
                }, ClickCallback.Options.builder().uses(1).build()));

        player.showDialog(Dialog.create(builder -> builder
                .empty()
                .base(base.build())
                .type(DialogType.multiAction(List.of(save, delete), createCancelButton(), 2))));
    }

    private void savePreset(Player player, Inventory inventory, String nameInput, String iconInput, String descriptionInput) {
        ...
        if (!repository.isValidName(name)) {
            this.showSaveDialog(
                    ...
                    iconName,
                    description,
                    "Use entre 1 y 32 caracteres: letras, números, espacios, _ o -.");
        ...
        if (icon == null) {
            this.showSaveDialog(
                    ...
                    iconName,
                    description,
                    "El icono \"%s\" no corresponde a un item válido.".formatted(iconName));
    ...
    private void updatePreset(
        ...
        getRepository().updateMetadata(preset.name(), icon, description);
        this.render(inventory);
        player.closeDialog();
        player.sendRichMessage("<green>Preset <white>%s</white> actualizado.".formatted(preset.name()));
    }
    ...
    private static ItemStack createPresetItem(PresetInfo preset) {
        ItemBuilder builder = new ItemBuilder(preset.icon()).name(TextFormatter.formatTinyCaps(preset.name()));

        if (!preset.description().isBlank()) {
            builder.addLore(Component.text(preset.description(), NamedTextColor.GRAY));
        }
    ...
    private static Material parseIcon(String input) {
        ...
        if (material == null && !value.contains(":")) {
            material = Material.matchMaterial("minecraft:" + value.toLowerCase(Locale.ROOT));
        }

        return PresetRepository.isValidIcon(material) ? material : null;
    }
